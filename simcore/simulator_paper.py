...
class MultiIngressPaperSimulator:
    ...
    def __init__(self,
        ...
        # === Trace Workload ===
        self.wl_inf = None
        if self.arr_inf.mode == 'trace' and self.arr_inf.trace_path:
            loader = AlibabaTrace(self.arr_inf.trace_path)
            jobs = loader.load()
            self.wl_inf = WorkloadManager(jobs)

        self.wl_trn = None
        if self.arr_trn.mode == 'trace' and self.arr_trn.trace_path:
            loader = AlibabaTrace(self.arr_trn.trace_path)
            jobs = loader.load()
            self.wl_trn = WorkloadManager(jobs)


        # schedule arrivals per ingress
        for ing in self.ingresses.values():
            if self.arr_inf.mode != 'trace':
                self._schedule(self.now + self.arr_inf.next_interarrival(self.now), 'arrival_inf', {'ing': ing.name})
            if self.arr_trn.mode != 'trace':
                self._schedule(self.now + self.arr_trn.next_interarrival(self.now), 'arrival_trn', {'ing': ing.name})
        
        # If trace, schedule first jobs (associating with first ingress for now)
        if self.ingresses:
            first_ing = list(self.ingresses.keys())[0]
            if self.wl_inf:
                job = self.wl_inf.get_next_job()
                if job:
                    self._schedule(job.arrival_time, 'arrival_inf', {'ing': first_ing, 'job': job})
            if self.wl_trn:
                job = self.wl_trn.get_next_job()
                if job:
                    self._schedule(job.arrival_time, 'arrival_trn', {'ing': first_ing, 'job': job})
    ...
    def _try_preempt_for_inference(self, dc: DataCenter):
        """Preempt training jobs to make room for inference if priority is enabled."""
        if not self.policy.inf_priority:
            return
        
        # Find all training jobs
        train_jobs = [(jid, job, g) for jid, (job, g) in dc.running_jobs.items() if job.jtype == 'training']
        if not train_jobs:
            return
            
        # Preempt the first one found (or could be latest/largest)
        # We only need enough GPUs for an inference job (usually small)
        jid, job, g = train_jobs[0]
        self._preempt_job(dc, job, "Preempted for inference priority")
        self.logger.info(f"[Priority] Preempted training job {jid} in {dc.name} to free {g} GPUs.")
    ...
    def run(self):
        ...
        while self.event_q:
            ...
            if etype == 'arrival_inf':
                self._handle_ingress_arrival('inference', payload['ing'], payload.get('job'))
            elif etype == 'arrival_trn':
                self._handle_ingress_arrival('training', payload['ing'], payload.get('job'))
    ...
    def _handle_ingress_arrival(self, jtype: str, ing_name: str, trace_job=None):
        ...
        if trace_job:
            wl = self.wl_inf if jtype == 'inference' else self.wl_trn
            next_j = wl.get_next_job()
            if next_j:
                self._schedule(next_j.arrival_time, 'arrival_inf' if jtype == 'inference' else 'arrival_trn', 
                               {'ing': ing_name, 'job': next_j})
        else:
    ...
    def _handle_transfer_done(self, payload: dict):
        ...
        # Preemption for Inference Priority
        if dc.free_gpus <= 0 and job.jtype == 'inference' and self.policy.inf_priority:
            self._try_preempt_for_inference(dc)
